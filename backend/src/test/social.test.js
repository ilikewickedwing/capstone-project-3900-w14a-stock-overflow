...
import { openFriendPf } from "../portfolio";
import { addStock } from "../stocks";
...
describe('Add friends', () => {
    ...
    it('Adding each other as friends', async() => {
        ...
        const pid = await getPid(token2, 'Watchlist', d);
        const add = await addStock(token2, pid, 'AAPL', null, null, null, null, d);
        const resp = await openFriendPf(token1, user2, d);
